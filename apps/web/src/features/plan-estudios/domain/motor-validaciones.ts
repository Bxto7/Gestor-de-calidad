 ...
 *
 * Espejo en frontend del `MotorDeValidaciones` que CLAUDE.md §3.4 sitúa en el
 * dominio del backend. Devuelve una lista estructurada de hallazgos, no lanza
 * excepciones sueltas: la UI necesita mostrarlos todos juntos (RF098), no
 * fallar en el primero.
 *
 * Vive aquí duplicado a propósito. La validación de servidor será la autoridad,
 * pero la UI tiene que poder anticipar el resultado sin un round-trip por cada
 * asignatura que el usuario arrastra.
 */
...
export function ciclosDeCarrera(carrera: Carrera): number[] {
...
export function validarPlan(entrada: EntradaValidacion): ResultadoValidacion {
  ...
  // Se valida también en los planes cerrados: quien decide cómo enmarcar los
  // hallazgos es la pantalla, que sabe si el plan admite correcciones. Callarlos
  // aquí haría que un plan histórico pareciese impecable.
