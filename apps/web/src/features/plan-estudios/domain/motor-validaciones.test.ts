...
function carrera(duracionAnios = 2): Carrera {
  return {
    id: 'car-1',
    facultadId: 'fac-1',
...
function plan(sobrescribir: Partial<PlanEstudios> = {}): PlanEstudios {
  return {
    id: 'plan-1',
    carreraId: 'car-1',
...
function asignatura(sobrescribir: Partial<Asignatura> = {}): Asignatura {
  return {
    id: 'asg-1',
    planId: 'plan-1',
    codigo: 'ISI-101',
...
describe('RF067 — total de créditos', () => {
  ...
  it('excluye las asignaturas inactivas', () => {
    ...
    const total = calcularTotalCreditos([
      asignatura({ id: 'a', creditos: 4 }),
      asignatura({ id: 'b', creditos: 3, estado: 'Inactivo' }),
    ]);
    expect(total).toBe(4);
  });
...
describe('RF094 — cada asignatura necesita una competencia', () => {
  ...
  it('nombra la asignatura afectada para poder corregirla', () => {
    ...
    // plan entero a mano.
    const e = entradaValida();
    e.asignaturas[1]!.competenciaIds = [];
    const r = validarPlan(e);
  ...
  it('ignora las asignaturas inactivas', () => {
    const e = entradaValida();
    e.asignaturas.push(
      asignatura({ id: 'x', codigo: 'ISI-199', competenciaIds: [], estado: 'Inactivo' }),
    );
...
describe('RF097 / RF098 — reporte consolidado', () => {
  it('acumula todos los problemas en una sola pasada', () => {
    ...
    e.asignaturas[0]!.competenciaIds = []; // RF094
