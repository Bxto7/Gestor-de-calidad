...
>
> Al 9 de septiembre de 2026: **158 de 243 requisitos contables construidos**,
...
> - **Citado sin construir** ya no tiene ejemplo vigente en este bloque:
>   `RF-PE-041` se cerró el 9 de septiembre de 2026 (ciclo 2c-D) — la
>   validación integral de consistencia reemplazó el placeholder de
>   `transicionar` que solo citaba el requisito sin implementarlo.
>
...
| Drag & drop | Drag and Drop API nativa del navegador, más un selector de ciclo por asignatura | Se eligió `@dnd-kit/core` y nunca se instaló: la malla usa la API nativa, como pide §4 del prompt de UI. Eso deja fuera lo que `@dnd-kit` daba resuelto —la operación por teclado—, y por eso cada asignatura lleva además un selector de ciclo, que RF061 exige como «selector alternativo para accesibilidad» y que ahí no es un extra sino la única vía sin ratón. Está explicado en la cabecera de `MallaCurricularPage.tsx` |
